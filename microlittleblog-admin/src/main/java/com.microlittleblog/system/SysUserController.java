...
public class SysUserController extends BaseController {
    ...
    public AjaxResult addSave(@Valid SysUser user) {
        if (UserConstants.USER_NAME_NOT_UNIQUE.equals(userService.checkUserUnique(SysUser.builder().loginName(user.getLoginName()).build()))) {
            return error("新增用户'" + user.getLoginName() + "'失败，登录账号已存在");
        } else if (UserConstants.USER_EMAIL_NOT_UNIQUE.equals(userService.checkUserUnique(SysUser.builder().email(user.getEmail()).build()))) {
